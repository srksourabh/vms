...
- Windows thin client lives in `desktop/` (green connection light). It does not start Docker; `installer/windows/Start-Server.bat` / `scripts/onprem-up.sh` do. `cd desktop && npm run pack:win` emits `desktop/dist/SecureGate-Portable.exe` (gitignored, ~67 MB PE32). Customer slides: `docs/customer/presentation.html`; pack: `docs/customer/QUEST-MALL-VMS-ONPREM.html`.
